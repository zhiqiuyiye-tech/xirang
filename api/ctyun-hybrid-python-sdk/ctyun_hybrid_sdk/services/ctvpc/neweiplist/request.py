...
class NewEipListRequest(CTYunRequest):
    """
    查询指定地域已创建的EIP。调用此接口可查询指定地域已创建的弹性公网IP（Elastic IP Address，简称EIP）。混合云入参缺失ipType和eipType两个字段，非必填字段，暂不做调整
    """

    def __init__(self, request_param):
        super(NewEipListRequest, self).__init__("/v4/eip/new-list", "POST", "ctvpc", "application/json")
...
class NewEipListRequestParam(object):
