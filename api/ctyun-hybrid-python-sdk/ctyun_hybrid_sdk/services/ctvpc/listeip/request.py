...
class ListEipRequest(CTYunRequest):
    """
    查询指定地域已创建的EIP。调用此接口可查询指定地域已创建的弹性公网IP（Elastic IP Address，简称EIP）。混合云入参缺失ipType和eipType两个字段，非必填字段，暂不做调整
    """

    def __init__(self, request_param):
        super(ListEipRequest, self).__init__("/v4/eip/list", "POST", "ctvpc", "application/json")
...
class ListEipRequestParam(object):

    def __init__(self, region_id, client_token=None, project_id=None, page=None, page_size=None, ids=None, ip_type=None, eip_type=None, status=None, resource_id=None, instance_id=None):
        """
