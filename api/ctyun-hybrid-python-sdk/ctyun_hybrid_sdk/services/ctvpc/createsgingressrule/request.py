...
class CreateSgIngressRuleRequest(CTYunRequest):
    """
    创建安全组入向规则。公有云入参缺clientToken字段，暂不修改
    """

    def __init__(self, request_param):
        super(CreateSgIngressRuleRequest, self).__init__("/v4/vpc/create-security-group-ingress", "POST", "ctvpc", "application/json")
...
class SecurityGroupRule(object):
    ...
    def __init__(self, direction, ethertype, priority, action, dest_cidr_ip, protocol, range=None, description=None):
        """
    ...
    def check_param(self):
        ...
        if self.dest_cidr_ip is None:
            raise Exception("dest_cidr_ip can not None")
        if self.protocol is None:
            raise Exception("protocol can not None")


class CreateSgIngressRuleRequestParam(object):
