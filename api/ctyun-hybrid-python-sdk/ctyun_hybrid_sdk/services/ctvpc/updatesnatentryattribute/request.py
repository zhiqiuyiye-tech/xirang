# coding=utf8

# Copyright 2023 CTYUN.CN
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

from ctyun_hybrid_sdk.core.request import CTYunRequest


class UpdateSnatEntryAttributeRequest(CTYunRequest):
    """
    修改SNAT规则
    """

    def __init__(self, request_param):
        super(UpdateSnatEntryAttributeRequest, self).__init__("/v4/vpc/modify-snat-entry", "POST", "ctvpc", "application/json")
        if request_param is None:
            raise Exception("request_param can not None")
        self.parameters = request_param
        self.parameters.check_param()
        self.header = dict()

    def get_body_param(self):
        """
        http body param get
        """
        body_param = dict()
        if self.parameters.region_id is not None:
            body_param["regionID"] = self.parameters.region_id
        if self.parameters.s_nat_id is not None:
            body_param["sNatID"] = self.parameters.s_nat_id
        if self.parameters.source_subnet_id is not None:
            body_param["sourceSubnetID"] = self.parameters.source_subnet_id
        if self.parameters.source_cidr is not None:
            body_param["sourceCIDR"] = self.parameters.source_cidr
        if self.parameters.client_token is not None:
            body_param["clientToken"] = self.parameters.client_token
        if self.parameters.description is not None:
            body_param["description"] = self.parameters.description
        if self.parameters.nat_gateway_id is not None:
            body_param["natGatewayID"] = self.parameters.nat_gateway_id
        return body_param

    def get_query_param(self):
        """
        http query param get
        """
        return dict()

    def get_path_param(self):
        """
        http path param get
        """
        return dict()


class UpdateSnatEntryAttributeRequestParam(object):

    def __init__(self, region_id, s_nat_id, source_subnet_id=None, source_cidr=None, client_token=None, description=None, nat_gateway_id=None):
        """
        :param region_id: 资源池id
        :param s_nat_id: SNAT条目id
        :param source_subnet_id: 子网id子网id，【非自定义情况且需要修改子网时必传】
        :param source_cidr: 自定义cidr，传子网时使用子网的cidr
        :param client_token: 客户端Token，用于保证请求的幂等性（非必填，并且此字段在私有云不具有实际意义）
        :param description: SNAT的描述信息 支持拉丁字母、中文、数字, 特殊字符：!@#$%^&*()+= <>?:"{},./;'[]·！@#￥%……&*（） —— -+_-={}|《》？：“”【】、；‘'，。、，长度 0 - 128。不支持换行符
        :param nat_gateway_id: NAT网关id
        """
        self.region_id = region_id
        self.s_nat_id = s_nat_id
        self.source_subnet_id = source_subnet_id
        self.source_cidr = source_cidr
        self.client_token = client_token
        self.description = description
        self.nat_gateway_id = nat_gateway_id

    def set_source_subnet_id(self, source_subnet_id):
        """
        :param source_subnet_id: 子网id子网id，【非自定义情况且需要修改子网时必传】
        """
        self.source_subnet_id = source_subnet_id

    def set_source_cidr(self, source_cidr):
        """
        :param source_cidr: 自定义cidr，传子网时使用子网的cidr
        """
        self.source_cidr = source_cidr

    def set_client_token(self, client_token):
        """
        :param client_token: 客户端Token，用于保证请求的幂等性（非必填，并且此字段在私有云不具有实际意义）
        """
        self.client_token = client_token

    def set_description(self, description):
        """
        :param description: SNAT的描述信息 支持拉丁字母、中文、数字, 特殊字符：!@#$%^&*()+= <>?:"{},./;'[]·！@#￥%……&*（） —— -+_-={}|《》？：“”【】、；‘'，。、，长度 0 - 128。不支持换行符
        """
        self.description = description

    def set_nat_gateway_id(self, nat_gateway_id):
        """
        :param nat_gateway_id: NAT网关id
        """
        self.nat_gateway_id = nat_gateway_id

    def check_param(self):
        """
        the param required check
        """
        if self.region_id is None:
            raise Exception("region_id can not None")
        if self.s_nat_id is None:
            raise Exception("s_nat_id can not None")

