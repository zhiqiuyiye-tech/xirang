...
class UpdateRouteTableRulesAttributeRequest(CTYunRequest):
    """
    批量修改路由表规则   3.0底层不支持批量修改，由云管做的兼容，先删除再创建，所以建议此接口批量个数尽量不超过5个
    """

    def __init__(self, request_param):
        super(UpdateRouteTableRulesAttributeRequest, self).__init__("/v4/vpc/route-table/modify-rules", "POST", "ctvpc", "application/json")
...
class RouteRule(object):
    ...
    def __init__(self, route_rule_id, next_hop_id, next_hop_type, ip_version, destination, description=None):
        """
        :param route_rule_id: 路由规则 id
        :param next_hop_id: 下一跳设备 id
        :param next_hop_type: vpcpeering（对等连接） / havip（虚拟IP） / bm（弹性裸金属服务器）/ vm（云主机） / natgw（NAT网关）/ igw6（IPv6网关） / dc（专线网关） / ticc（云间高速） / vpngw（VPN网关） / enic（弹性网卡）   
         目前4.0IPV4只支持云主机、裸金属、弹性网卡、虚拟ip、vpn网关、Nat网关、对等连接、IPV4网关   
         4.0 IPV6支持云主机、裸金属、虚拟IP、对等连接、IPV6网关   
         3.0ipv4只支持云主机、nat网关、虚拟IP、对等连接、专线网关，3.0 ipv6只支持云主机、虚拟IP
    ...
    def get_dic(self):
        ...
        if self.route_rule_id is not None:
            obj_dict["routeRuleID"] = self.route_rule_id
...
class UpdateRouteTableRulesAttributeRequestParam(object):
