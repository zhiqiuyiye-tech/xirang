...
class NewEipListAuditRequest(CTYunRequest):
    """
    查询指定地域已创建的EIP。调用此接口可查询指定地域已创建的弹性公网IP（Elastic IP Address，简称EIP）。混合云入参缺失ipType非必填字段，暂不做调整
    """

    def __init__(self, request_param):
        super(NewEipListAuditRequest, self).__init__("/v4/eip/new-list-audit", "POST", "ctvpc", "application/json")
...
class NewEipListAuditRequestParam(object):

    def __init__(self, region_id, client_token=None, project_id=None, page=None, page_size=None, ids=None, ip_type=None, eip_type=None, status=None, resource_id=None, ip=None):
        """
        :param region_id: 资源池ID
        :param client_token: 客户端存根，用于保证订单幂等性。要求单个云平台账户内唯一（实际用不到）
        :param project_id: 企业项目 ID
        :param page: 分页参数，输入0或不填默认是1
        :param page_size: 每页数据量大小，取值 1-50，超过50默认为50，不填默认10。注：查询全量可填-1
        ...
        :param eip_type: eip类型:xinchuang/internet/cn2/bgp-3/chinamobile/chinaunicom;   
         3.0 ext-net
        :param status: 绑定网卡状态 DOWN 未绑定, ACTIVE 绑定 大小写适配
        :param resource_id: 资源id（当resourceId与ids同时传时取并集）
        :param ip: eip IP地址，模糊搜索，不校验Ip的有效性，只校验EIP是否存在
        """
