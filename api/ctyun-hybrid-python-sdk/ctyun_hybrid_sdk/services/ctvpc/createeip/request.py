...
class CreateEipRequest(CTYunRequest):
    ...
    def __init__(self, request_param):
        super(CreateEipRequest, self).__init__("/v4/eip/create", "POST", "ctvpc", "application/json")
...
class CreateEipRequestParam(object):
    ...
    def __init__(self, region_id, name, cycle_type, client_token=None, cycle_count=None, bandwidth=None, bandwidth_id=None, demand_billing_type=None, provider=None, project_id=None):
        """
        :param client_token: 客户端存根，用于保证订单幂等性。要求单个云平台账户内唯一
        :param region_id: 资源池 ID
        :param name: 弹性 IP 名称（2-32长度，只允许中文、英文、数字和特殊字符-, _，只能以中文和英文开头），名称不能重复
        ...
        :param bandwidth: 弹性 IP 的带宽峰值，传0或者不传时默认为 1 Mbps，峰值带宽上限默认最大上限值是3000，若云管配置中心设置的带宽上限值大于3000，则以云管设置的参数上限为准
        :param bandwidth_id: 当 cycleType 为 on_demand 时，可以使用 bandwidthID，将弹性 IP 加入到共享带宽中
        :param demand_billing_type: 按需计费类型，当 cycleType 为 on_demand 时生效，支持 bandwidth（按带宽）/ upflowc（按流量），默认是按带宽
