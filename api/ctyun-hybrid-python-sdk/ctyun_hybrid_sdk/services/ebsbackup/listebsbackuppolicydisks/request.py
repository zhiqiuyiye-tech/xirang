# coding=utf8

# Copyright 2023 CTYUN.CN
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

from ctyun_hybrid_sdk.core.request import CTYunRequest


class ListEbsBackupPolicyDisksRequest(CTYunRequest):
    """
    diskMode（云硬盘模式） 未对齐
    """

    def __init__(self, request_param):
        super(ListEbsBackupPolicyDisksRequest, self).__init__("/v4/ebs-backup/policy/list-disks", "GET", "ebsbackup", "")
        if request_param is None:
            raise Exception("request_param can not None")
        self.parameters = request_param
        self.parameters.check_param()
        self.header = dict()

    def get_body_param(self):
        """
        http body param get
        """
        return dict()

    def get_query_param(self):
        """
        http query param get
        """
        query_param = dict()
        if self.parameters.region_id is not None:
            query_param["regionID"] = self.parameters.region_id
        if self.parameters.policy_id is not None:
            query_param["policyID"] = self.parameters.policy_id
        if self.parameters.page_no is not None:
            query_param["pageNo"] = self.parameters.page_no
        if self.parameters.page_size is not None:
            query_param["pageSize"] = self.parameters.page_size
        if self.parameters.disk_id is not None:
            query_param["diskID"] = self.parameters.disk_id
        if self.parameters.disk_name is not None:
            query_param["diskName"] = self.parameters.disk_name
        return query_param

    def get_path_param(self):
        """
        http path param get
        """
        return dict()


class ListEbsBackupPolicyDisksRequestParam(object):

    def __init__(self, region_id, policy_id, page_no=None, page_size=None, disk_id=None, disk_name=None):
        """
        :param region_id: 资源池id
        :param policy_id: 备份策略ID
        :param page_no: 页码，默认值1
        :param page_size: 每页记录数目 ,默认10，范围[1-100]，大于100取100，不传、传0取10
        :param disk_id: 云硬盘ID,支持模糊过滤
        :param disk_name: 云硬盘名称，模糊过滤，指定diskID时，该参数无效
        """
        self.region_id = region_id
        self.policy_id = policy_id
        self.page_no = page_no
        self.page_size = page_size
        self.disk_id = disk_id
        self.disk_name = disk_name

    def set_page_no(self, page_no):
        """
        :param page_no: 页码，默认值1
        """
        self.page_no = page_no

    def set_page_size(self, page_size):
        """
        :param page_size: 每页记录数目 ,默认10，范围[1-100]，大于100取100，不传、传0取10
        """
        self.page_size = page_size

    def set_disk_id(self, disk_id):
        """
        :param disk_id: 云硬盘ID,支持模糊过滤
        """
        self.disk_id = disk_id

    def set_disk_name(self, disk_name):
        """
        :param disk_name: 云硬盘名称，模糊过滤，指定diskID时，该参数无效
        """
        self.disk_name = disk_name

    def check_param(self):
        """
        the param required check
        """
        if self.region_id is None:
            raise Exception("region_id can not None")
        if self.policy_id is None:
            raise Exception("policy_id can not None")

