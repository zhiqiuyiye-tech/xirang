...
class EbsBackupPolicyBindVolumesRequest(CTYunRequest):
    """
    云硬盘只能绑定一个备份策略，如果一个云硬盘已经绑定了备份策略，再次调用该接口绑定新的备份策略，旧的备份策略会自动解绑。云硬盘和备份策略需要是同一所属用户。
    """

    def __init__(self, request_param):
        super(EbsBackupPolicyBindVolumesRequest, self).__init__("/v4/ebs-backup/policy/bind-volumes", "POST", "ebsbackup", "application/json")
...
class EbsBackupPolicyBindVolumesRequestParam(object):
