...
class CreateBackupRequest(CTYunRequest):
    """
    开发未对齐原因：混合云返回异步任务ID，公有云返回云硬盘备份对象
    """

    def __init__(self, request_param):
        super(CreateBackupRequest, self).__init__("/v4/ebs-backup/create-backup", "POST", "ebsbackup", "application/json")
...
class CreateBackupRequestParam(object):

    def __init__(self, region_id, repository_id, backup_name, disk_id=None, resource_id=None, description=None):
        """
        :param region_id: 资源池id
        :param disk_id: 云硬盘ID 参数resourceID或diskID二者必传其一(二者同时存在以diskID为主)
        :param resource_id: 资源ID 参数resourceID或diskID二者必传其一(二者同时存在以diskID为主)
