...
class MetadataBatchCreateRequest(CTYunRequest):
    """
    只有弹性裸金属支持元数据
    """

    def __init__(self, request_param):
        super(MetadataBatchCreateRequest, self).__init__("/v4/ebm/metadata/batch-create", "POST", "ebm", "application/json")
...
class MetadataBatchCreateRequestParam(object):

    def __init__(self, region_id, instance_uuid, metadata, az_name=None):
        """
        :param region_id: 区域ID
        :param az_name: 可用区名称或者 default。您可以调用 https://eop.ctyun.cn/ebp/ctapiDocument/search?sid=25&api=5855&data=87 获取资源池信息，查询结果中zoneList内返回可用区名称，若查询结果中zoneList为空，即为单可用区，本字段填写default。
