...
class QueryCreateLbPriceRequest(CTYunRequest):
    """
    v2版本询价接口有效参数仅有regionID，slaName，cycleType，cycleCount参数，其余参数仅在必传性兼容公有云，不校验正确性
    """

    def __init__(self, request_param):
        super(QueryCreateLbPriceRequest, self).__init__("/v4/elb/query-create-price", "POST", "ctelb", "application/json")
...
class QueryCreateLbPriceRequestParam(object):

    def __init__(self, client_token, region_id, subnet_id, name, sla_name, resource_type, cycle_type, cycle_count, project_id=None, vpc_id=None, description=None, eip_id=None, private_ip_address=None):
        """
        :param client_token: 公有云文档必传
        :param region_id: 资源池id
        :param project_id: 云管不做处理
        :param vpc_id: 专有网络ID
        ...
        :param cycle_type: 订购类型：month（包月） / year（包年），不传默认为按需
