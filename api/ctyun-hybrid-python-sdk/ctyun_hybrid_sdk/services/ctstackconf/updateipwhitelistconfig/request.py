...
class UpdateIPWhiteListConfigRequest(CTYunRequest):
    """
    登录IP白名单修改
    """

    def __init__(self, request_param):
        super(UpdateIPWhiteListConfigRequest, self).__init__("/v1/login-secure-whitelist/update", "POST", "ctstackconf", "application/json")
...
class UpdateIPWhiteListConfigRequestParam(object):

    def __init__(self, id, ip, desc=None):
        """
        :param id: ID 编号
        :param ip: 登录ip
        :param desc: 描述信息，字符串最大长度128
        """
        self.id = id
        self.ip = ip
        self.desc = desc
