...
class ResizeOceanfsSfsRequest(CTYunRequest):
    """
    海量文件修改规格
    """

    def __init__(self, request_param):
        super(ResizeOceanfsSfsRequest, self).__init__("/v4/oceanfs/resize-sfs", "POST", "oceanfs", "application/json")
...
class ResizeOceanfsSfsRequestParam(object):

    def __init__(self, region_id, sfs_size, client_token=None, sfs_uid=None, resource_id=None):
        """
        :param client_token: 客户端存根，用于保证订单幂等性
        :param region_id: 资源池 ID
        :param sfs_size: 变配后的大小，单位 GB
        :param sfs_uid: sfsUID和reourceID必填一个，同时存在以sfsUID为准
        :param resource_id: sfsUID和reourceID必填一个，同时存在以sfsUID为准
        """
