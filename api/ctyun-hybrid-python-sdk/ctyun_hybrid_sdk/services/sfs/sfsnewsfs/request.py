...
class SfsNewSfsRequest(CTYunRequest):
    """
    ...
    def __init__(self, request_param):
        super(SfsNewSfsRequest, self).__init__("/v4/sfs/new-sfs", "POST", "sfs", "application/json")
...
class SfsNewSfsRequestParam(object):

    def __init__(self, region_id, sfs_type, sfs_protocol, sfs_name, sfs_size, az_name=None, client_token=None, is_encrypt=None, kms_uuid=None, project_id=None, on_demand=None, cycle_type=None, cycle_count=None, vpc=None, subnet=None):
        """
        ...
        :param sfs_name: 弹性文件名，命名规则：2-63字符限制，支持使用字母、数字、中划线（-），只能以字母开头、以数字或字母结尾
        :param sfs_size: 大小，单位 GB,最小100GB，最大327680GB
        :param on_demand: 是否按需下单。true/false，默认为 false；暂不与公有云对齐
        :param cycle_type: 包周期（subscription）类型，year/month。onDemand 为 false 时，必须指定
        :param cycle_count: 包周期数。onDemand 为 false 时必须指定。周期最大长度不能超过5年/60个月
