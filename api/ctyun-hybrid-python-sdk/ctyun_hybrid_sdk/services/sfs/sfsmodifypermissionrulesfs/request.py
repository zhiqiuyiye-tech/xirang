...
class SfsModifyPermissionRuleSfsRequest(CTYunRequest):
    """
    1. 优先级不能重复   
    2. 授权地址不能重复   
    ...
    def __init__(self, request_param):
        super(SfsModifyPermissionRuleSfsRequest, self).__init__("/v4/sfs/permission-rule/modify-permission-rule", "POST", "sfs", "application/json")
...
class SfsModifyPermissionRuleSfsRequestParam(object):

    def __init__(self, region_id, permission_rule_fuid, rw_permission, user_permission, auth_addr, permission_rule_priority, ):
        """
        :param region_id: 资源池ID
        :param permission_rule_fuid: 权限组规则ID
        :param rw_permission: 有效值范围： rw、ro
        :param user_permission: 有效值范围：no_root_squash
        :param auth_addr: 授权地址
        :param permission_rule_priority: 最小值为1，最大值为400
        """
    ...
    def check_param(self):
        ...
        if self.permission_rule_fuid is None:
            raise Exception("permission_rule_fuid can not None")
