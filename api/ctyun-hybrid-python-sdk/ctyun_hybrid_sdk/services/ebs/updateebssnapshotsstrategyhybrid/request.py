...
class UpdateEbsSnapshotsStrategyHybridRequest(CTYunRequest):
    """
    快照策略 修改。   
    **注意**：（开发未对齐）公有云无此接口，按照py接口开发
    ...
    def __init__(self, request_param):
        super(UpdateEbsSnapshotsStrategyHybridRequest, self).__init__("/v4/ebs/snapshots_strategy/update", "POST", "ebs", "application/json")
    ...
    def get_body_param(self):
        ...
        if self.parameters.snapshoot_rule_id is not None:
            body_param["snapshootRuleId"] = self.parameters.snapshoot_rule_id
        return body_param
...
class UpdateEbsSnapshotsStrategyHybridRequestParam(object):

    def __init__(self, region_id, cycle_type, frequency, is_enable, name, obj_type, remain_first_backup_of_cur_month, rentention_num, rentention_type, snapshoot_rule_id, ):
        """
        :param region_id: 资源池id
        :param cycle_type: 备份周期类型 day or week
        :param frequency: 执行频率。cycle_type 等于 week 时，表示周几执 行，多个用逗号分割，0-6:星期日-星期一；cycle_type 等于 day 时，表示每隔多少天执行。
        ...
        :param snapshoot_rule_id: 要修改的快照策略ID
        """
        ...
        self.snapshoot_rule_id = snapshoot_rule_id
    ...
    def check_param(self):
        ...
        if self.snapshoot_rule_id is None:
            raise Exception("snapshoot_rule_id can not None")
