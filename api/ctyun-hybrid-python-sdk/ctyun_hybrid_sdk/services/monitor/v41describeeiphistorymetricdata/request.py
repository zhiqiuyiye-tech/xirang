...
class V41describeEIPHistoryMetricDataRequest(CTYunRequest):
    """
    ...
    def __init__(self, request_param):
        super(V41describeEIPHistoryMetricDataRequest, self).__init__("/v4.1/monitor/query-eip-historymetricdata", "POST", "monitor", "application/json")
...
class V41describeEIPHistoryMetricDataRequestParam(object):

    def __init__(self, region_id, device_uuid_list, item_name_list, start_time, end_time, period=None):
        """
        :param region_id: 资源池ID
        :param device_uuid_list: 查询设备ID列表(建议:设备数不大于20，可以重复)注：弹性IP设备ID传IP地址(例:100.127.176.100) 注意:此参数为数组
