...
class CreateResourceGroupRequest(CTYunRequest):
    """
    1. 资源分组名称不可重复。2. 可创建的资源分组数量不能超出配额数量。3. 其他参见请求参数说明。
    """

    def __init__(self, request_param):
        super(CreateResourceGroupRequest, self).__init__("/v4.1/monitor/create-resource-group", "POST", "monitor", "application/json")
...
class CreateResourceGroupRequestParam(object):

    def __init__(self, region_id, name, desc=None, create_type=None, resource_list=None, project_info=None):
        """
        :param region_id: 资源池ID
        :param name: 长度为2-63个字符，中文、英文（大小写）、数字、点号 (.)、下划线(_)、半角冒号 (:)、连字符 (-)，不支持连续字符--
        :param desc: 描述
        :param create_type: 本参数表示创建方式，默认值：instance。取值范围： instance：实例创建。 project：企业项目创建。 根据以上范围取值。
