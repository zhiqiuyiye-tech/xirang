...
class QueryMetricDataAvgHybridRequest(CTYunRequest):
    """
    查询某个设备的历史数据，以period为周期，返回每个周期内的平均值
    """

    def __init__(self, request_param):
        super(QueryMetricDataAvgHybridRequest, self).__init__("/v4/monitor/query-metricdata-avg", "POST", "monitor", "application/json")
...
class QueryMetricDataAvgHybridRequestParam(object):
    ...
    def __init__(self, region_id, device_type, item_name_list, device_uuid_list, start_time, end_time, page_no=None, page_size=None, period=None):
        ...
        :param device_type: 本参数表示设备类型。取值范围：vm：云主机。ph：宿主机。bare_metal：裸金属。disk：云磁盘。scaling：弹性伸缩。traffic：共享带宽。eip：弹性IP。elb：负载均衡。listener：监听器。cstor_sfs：弹性文件。
