...
class UpdateAlarmRuleInfoV41Request(CTYunRequest):
    """
    调用此接口可修改指定告警规则基本信息，支持全量字段修改。
    """

    def __init__(self, request_param):
        super(UpdateAlarmRuleInfoV41Request, self).__init__("/v4.1/monitor/update-alarm-rule", "POST", "monitor", "application/json")
...
class Condition(object):
    ...
    def __init__(self, evaluation_count, metric, fun, operator, value, period, unit=None, level=None):
        """
        :param evaluation_count: 当规则执行结果持续多久符合条件时报警（防抖）。（云管只支持1-5次）
        :param metric: 监控指标
...
class UpdateAlarmRuleInfoV41RequestParam(object):
    ...
    def __init__(self, alarm_rule_id, region_id, name, service, dimension, conditions, desc=None, repeat_times=None, silence_time=None, recover_notify=None, notify_type=None, contact_group_list=None, notify_weekdays=None, notify_start=None, notify_end=None, webhook_url=None, res_group_id=None, resources=None, project_id=None, condition_type=None, resource_scope=None, default_contact=None, notice_strategy_id=None):
        ...
        :param dimension: 本参数表示告警维度。取值范围：   
         ...
         physical_line：物理专线；   
         pushgateway_hpfs：并行文件服务
        :param conditions: 具体匹配规则 注意:此参数为数组
        :param desc: 描述
        ...
        :param notify_type: 本参数表示告警接收策略。取值范围：   
         ...
         sms：短信告警。   
         根据以上范围取值。 注意:此参数为数组
        :param contact_group_list: (仅支持一个) 注意:此参数为数组
    ...
    def set_contact_group_list(self, contact_group_list):
        """
        :param contact_group_list: (仅支持一个)
        """
        self.contact_group_list = contact_group_list
