...
class V4PaasEcsSnapshotCreateInstanceRequest(CTYunRequest):
    """
    快照创建云主机(工单)   
    - 至少存在一个快照   
    - 快照的状态为可用   
    ...
    def __init__(self, request_param):
        super(V4PaasEcsSnapshotCreateInstanceRequest, self).__init__("/v4/paas/ecs/snapshot/create-instance", "POST", "ct", "application/json")
...
class ChannelInfo(object):
    ...
    def __init__(self, paas_accout_id=None, master_order_id=None, paas_resource_id=None):
        """
...
class V4PaasEcsSnapshotCreateInstanceRequestParam(object):

    def __init__(self, client_token, region_id, instance_name, display_name, snapshot_id, vpc_id, network_card_list, ext_ip, sec_group_list=None, ip_version=None, bandwidth=None, eip_id=None, affinity_group_id=None, key_pair_id=None, user_password=None, user_data=None, project_id=None, channel_info=None):
        """
        ...
        :param ip_version: （暂不支持）取值范围:<br />ipv4：v4地址，<br />ipv6：v6地址
        :param bandwidth: 带宽大小单位为Mbit/s，取值范围:[1~2000]，extIP为1时必传
        :param eip_id: 弹性公网IP的ID（orderCount>1时不可用）
        :param affinity_group_id: 云主机组ID
        :param key_pair_id: 暂不支持
        :param user_password: （暂不支持）满足以下规则：<br />长度在8～30个字符;<br />必须包含大写字母、小写字母、数字以及特殊符号中的三项;<br />特殊符号可选：()`~!@#$%^&*_-+=｜{}[]:;'<>,.?/\\且不能以斜线号 / 开头
        :param user_data: 需要以Base64方式编码,Base64编码后的长度限制为1-16384字符
        :param project_id: 默认值为"0"
        :param channel_info: 暂不支持
        """
