...
class V4PaasEipModifySpecRequest(CTYunRequest):
    """
    用于修改弹性公网 IP 带宽峰值：   
    ...
    def __init__(self, request_param):
        super(V4PaasEipModifySpecRequest, self).__init__("/v4/paas/eip/modify-spec", "POST", "ct", "application/json")
...
class V4PaasEipModifySpecRequestParam(object):

    def __init__(self, region_id, eip_id, bandwidth, client_token=None, project_id=None, channel_info=None):
        """
        :param client_token: 客户端存根，用于保证订单幂等性。要求单个云平台账户内唯一（实际上没用到）(差异点说明：2.0暂无此参数)
        :param region_id: 资源池ID
        :param project_id: 企业项目ID
        :param eip_id: 弹性 IP id
        :param bandwidth: 弹性 IP 带宽，下限需大于0，峰值带宽上限默认最大上限值是3000，若云管配置中心设置的带宽上限值大于3000，则以云管设置的参数上限为准
        :param channel_info: 渠道侧信息，暂时空不支持。后续完善
        """
