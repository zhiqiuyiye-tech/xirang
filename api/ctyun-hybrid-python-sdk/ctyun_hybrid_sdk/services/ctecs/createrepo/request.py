...
class CreateRepoRequest(CTYunRequest):
    ...
    def __init__(self, request_param):
        super(CreateRepoRequest, self).__init__("/v4/ecs/backup-repo/create", "POST", "ctecs", "application/json")
...
class CreateRepoRequestParam(object):
    ...
    def __init__(self, client_token, region_id, repository_name, size, cycle_type, cycle_count=None, auto_renew_status=None, project_id=None):
        """
        :param client_token: 客户端存根（混合云非必填）
        :param region_id: 资源池id
        :param repository_name: 云主机备份存储库名称
        :param size: 云主机备份存储库容量
        :param cycle_type: 本参数表示订购周期类型 ，取值范围： MONTH：按月 YEAR：按年   
         包年包月需必传cycleCount的属性   
         按需计费cycleType=onDemand
        :param cycle_count: 订购时长，与cycleType配合，cycleType为Month时，单位为月，cycleType为YEAR时，单位为年
        :param auto_renew_status: (该参数暂不支持)本参数表示是否自动续订 ，取值范围： 0：不续费 1：自动续费，默认不自动续费
