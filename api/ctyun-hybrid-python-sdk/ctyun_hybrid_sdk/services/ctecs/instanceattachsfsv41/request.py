...
class InstanceAttachSfsV41Request(CTYunRequest):
    """
    ...
    def __init__(self, request_param):
        super(InstanceAttachSfsV41Request, self).__init__("/v4/ecs/sfs/attach", "POST", "ctecs", "application/json")
...
class SysInfo(object):
    ...
    def __init__(self, file_sys_id, file_sys_route, mount_point, option, protocol, auto_mount=None):
        """
        :param file_sys_id: 文件系统id
        :param file_sys_route: 文件系统地址（固定值，每一个文件都有相对应的文件系统地址）
        :param mount_point: 挂载点
        :param option: 挂载参数，以mount命令说明为准，如unlock-禁用文件锁机制；vers=3支持NfsV3协议；timeo设置超时时间(e.g. vers=3,timeo=600,nolock)
        :param auto_mount: 是否开机自动挂载(true/false)，当云主机重启时会自动挂载文件系统，默认为false	
    ...
    def set_auto_mount(self, auto_mount):
        """
        :param auto_mount: 是否开机自动挂载(true/false)，当云主机重启时会自动挂载文件系统，默认为false	
        """
        self.auto_mount = auto_mount
    ...
    def get_dic(self):
        obj_dict = dict()
        if self.file_sys_id is not None:
            obj_dict["fileSysID"] = self.file_sys_id
        ...
        if self.option is not None:
            obj_dict["option"] = self.option
        if self.auto_mount is not None:
            obj_dict["autoMount"] = self.auto_mount
    ...
    def check_param(self):
        ...
        if self.option is None:
            raise Exception("option can not None")
        if self.protocol is None:
            raise Exception("protocol can not None")


class InstanceAttachSfsV41RequestParam(object):

    def __init__(self, region_id, instance_id, sys_info_list, ):
        """
        :param region_id: 资源池ID
        :param instance_id: 云主机ID
        :param sys_info_list: 所绑定的文件系统详细信息 注意:此参数为数组
        """
