...
class ListFlavorRequest(CTYunRequest):
    """
    ...
    **注意**： 如果您传了flavorID 则azName 为必填。如果只传regionID 则可查询所有数据，azName不是必填的   
    ...
    def __init__(self, request_param):
        super(ListFlavorRequest, self).__init__("/v4/ecs/flavor/list", "POST", "ctecs", "application/json")
...
class ListFlavorRequestParam(object):

    def __init__(self, region_id, az_name=None, flavor_id=None, flavor_type=None, flavor_name=None, flavor_cpu=None, flavor_ram=None, flavor_arch=None, flavor_series=None, nic_count=None, dec_id=None, instance_id=None):
        """
        :param region_id: 资源池ID
        :param az_name: 支持多az的资源池，可用区必填
        ...
        :param flavor_series: 规格系列   取值范围：s：通用型，c：计算增强型，m：内存优化型，hs：海光通用型，hc：海光计算增强型，hm：海光内存型，fs：飞腾通用型，fc：飞腾计算增强型，fm：飞腾内存优化型，ks：鲲鹏通用型，kc：鲲鹏计算增强型，km：鲲鹏内存优化型，g：GPU图形加速基础型，p：GPU计算加速型，ip3：超高IO型，kir3：鲲鹏超高IO型
        :param nic_count: 网卡个数（暂不支持）
        :param dec_id: 查询指定专属云下关联的云主机规格
        :param instance_id: 支持过滤支持变配的规格
        """
