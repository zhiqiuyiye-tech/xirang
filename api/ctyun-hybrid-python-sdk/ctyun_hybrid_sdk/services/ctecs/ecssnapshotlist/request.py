# coding=utf8

# Copyright 2023 CTYUN.CN
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

from ctyun_hybrid_sdk.core.request import CTYunRequest


class EcsSnapshotListRequest(CTYunRequest):
    """
    查询云主机快照列表
    """

    def __init__(self, request_param):
        super(EcsSnapshotListRequest, self).__init__("/v4/ecs/snapshot-list", "POST", "ctecs", "application/json")
        if request_param is None:
            raise Exception("request_param can not None")
        self.parameters = request_param
        self.parameters.check_param()
        self.header = dict()

    def get_body_param(self):
        """
        http body param get
        """
        body_param = dict()
        if self.parameters.region_id is not None:
            body_param["regionID"] = self.parameters.region_id
        if self.parameters.page_no is not None:
            body_param["pageNo"] = self.parameters.page_no
        if self.parameters.page_size is not None:
            body_param["pageSize"] = self.parameters.page_size
        if self.parameters.sort is not None:
            body_param["sort"] = self.parameters.sort
        if self.parameters.asc is not None:
            body_param["asc"] = self.parameters.asc
        if self.parameters.id is not None:
            body_param["ID"] = self.parameters.id
        if self.parameters.snapshot_id is not None:
            body_param["snapshotID"] = self.parameters.snapshot_id
        if self.parameters.name is not None:
            body_param["name"] = self.parameters.name
        if self.parameters.keyword is not None:
            body_param["keyword"] = self.parameters.keyword
        if self.parameters.status is not None:
            body_param["status"] = self.parameters.status
        return body_param

    def get_query_param(self):
        """
        http query param get
        """
        return dict()

    def get_path_param(self):
        """
        http path param get
        """
        return dict()


class EcsSnapshotListRequestParam(object):

    def __init__(self, region_id, page_no=None, page_size=None, sort=None, asc=None, id=None, snapshot_id=None, name=None, keyword=None, status=None):
        """
        :param region_id: 区域ID
        :param page_no: 页码，默认值:1
        :param page_size: 每页记录数目，取值范围：[1, 100]，超过100按100处理
        :param sort: 排序关键字, 范围：created_time, updated_time，其他无效
        :param asc: 排序方向，默认false升序，true降序
        :param id: 云主机实例ID
        :param snapshot_id: 云主机快照实例ID
        :param name: 快照名称
        :param keyword: 查询内容
        :param status: 快照状态  公有云字段，混合云暂不支持，忽略
        """
        self.region_id = region_id
        self.page_no = page_no
        self.page_size = page_size
        self.sort = sort
        self.asc = asc
        self.id = id
        self.snapshot_id = snapshot_id
        self.name = name
        self.keyword = keyword
        self.status = status

    def set_page_no(self, page_no):
        """
        :param page_no: 页码，默认值:1
        """
        self.page_no = page_no

    def set_page_size(self, page_size):
        """
        :param page_size: 每页记录数目，取值范围：[1, 100]，超过100按100处理
        """
        self.page_size = page_size

    def set_sort(self, sort):
        """
        :param sort: 排序关键字, 范围：created_time, updated_time，其他无效
        """
        self.sort = sort

    def set_asc(self, asc):
        """
        :param asc: 排序方向，默认false升序，true降序
        """
        self.asc = asc

    def set_id(self, id):
        """
        :param id: 云主机实例ID
        """
        self.id = id

    def set_snapshot_id(self, snapshot_id):
        """
        :param snapshot_id: 云主机快照实例ID
        """
        self.snapshot_id = snapshot_id

    def set_name(self, name):
        """
        :param name: 快照名称
        """
        self.name = name

    def set_keyword(self, keyword):
        """
        :param keyword: 查询内容
        """
        self.keyword = keyword

    def set_status(self, status):
        """
        :param status: 快照状态  公有云字段，混合云暂不支持，忽略
        """
        self.status = status

    def check_param(self):
        """
        the param required check
        """
        if self.region_id is None:
            raise Exception("region_id can not None")

